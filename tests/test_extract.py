...
class TestLoadNEOs(unittest.TestCase):
    """ NEO extraction tests. """

    @classmethod
    def setUpClass(cls):
        cls.neos = load_neos(cfg.TEST_NEO_FILE)
...
class TestLoadApproaches(unittest.TestCase):
    """ CloseApproach extraction tests. """

    @classmethod
    def setUpClass(cls):
        cls.approaches = load_approaches(cfg.TEST_CAD_FILE)
    ...
    def test_approach_time_is_datetime(self):
        approach = self.get_first_approach_or_none()
        self.assertIsNotNone(approach)
        self.assertIsInstance(approach.time, datetime)
