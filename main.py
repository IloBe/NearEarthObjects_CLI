...
Additional note:
If you use the inspect call without further arguments, you will get a (neo) sign as your
command line prompt. With the 'help' argument you get the following list of options:
EOF exit help i inspect q query quit.
E.g. then calling i --name Halley results in getting the Halley string information about
its fullname, its diameter and not being potentially hazardous.
...
# transfered this original coding to own config file
# as central point to collect project constants and simple logger
...
def inspect(database, pdes=None, name=None, verbose=False):
    ...
    if not neo:
        LOGGER.info("No matching NEOs exist in the database.")
...
def query(database, args):
    ...
    LOGGER.info(f"Created filters for query():\n{filters}")
    results = database.query(filters)

    if not args.outfile:
        # Write the results to stdout, limiting to 10 entries if not specified.
        LOGGER.info("No outfile available, write results to stdout...")
        for result in limit(results, args.limit or 10):
            print(result)
    else:
        # Write the results to a file.
        LOGGER.info(f"Outfile available, write results to: {args.outfile}")
...
class NEOShell(cmd.Cmd):
    ...
    def parse_arg_with(cls, arg, parser):
        ...
        try:
            ...
            LOGGER.error(
                f"NEOShell parse_arg_with: ValueError: Return None:\n{err}",
                exc_info=True)
            return None
        ...
        try:
            ...
            LOGGER.error(f"===> System exit with:\n{err}", exc_info=True)
            return None
