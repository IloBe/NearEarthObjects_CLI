...
Concretely, you'll have to read data from both a CSV file and a JSON file, convert that data into structured Python objects, perform filtering operations on the data, limit the size of the result set, and write the results to a file in a structured format, such as CSV or JSON. Note: The JSON file is added as a zip file and has to be unzipped before usage. The original file is too big for the projects GitHub data directory.
...
Curios? If you want to know a little bit more general information about NASA missions and knowledge, like what is the difference between a comet and an asteroid, here is the [page](https://www.nasa.gov/feature/episode-16-we-asked-a-nasa-scientist-what-s-the-difference-between-asteroids-comets-and) for you.
...
.
├── README.md       # This file.
├── main.py         # Partly modified file
├── config.py       # Own created file for constants and logger component
├── models.py       # Task 1.
├── read.py         # Task 2a.
...
│   ├── neos.csv
│   └── cad.json
├── logs            # Own created directory for log file storage; log files are not part of the git repository
└── tests
    ├── test-neos-2020.csv
    ├── test-cad-2020.json
    ├── test_*.py
...
- `main.py`: The main Python script that wraps the command-line tool, orchestrates the data pipeline by invoking the functions and classes that you'll write. **You will not need to modify this file.** But I did, moved constants to a newly created module `config.py` and added `logging` behaviour.
